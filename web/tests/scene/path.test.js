...
const SCENE = { x: 240, y: -50,  length:  80, width: 20, orientation: 'x', dir: { path: 'src/scene' } };
...
describe('streetChainForDirPath', () => {
  ...
  it('walks three-deep chain root-first', () => {
    expect(streetChainForDirPath('src/scene', STREETS))
      .toEqual([ROOT, SRC, SCENE]);
  });
...
describe('computePathPoints', () => {
  ...
  it('directory selection three-deep produces 4 segments (chain walks through all parents)', () => {
    ...
    const INNER = { x: 245, y: -75, length: 20, width: 10, orientation: 'y', dir: { path: 'src/scene/inner' } };
