...
- [ ] (QA) `/api/manifest/signature` returns 400 on the live-update poll for git URLs (seen with nanostores@main, repeating 4×). The 400 is `resolve_source(src, branch)` raising `ResolveError(400)` in the signature route — likely the poll firing before the clone/resolve completes (timing race) or a resolve failure for that repo. Unrelated to building settings. Decide whether the poll should hold off until the repo is loaded. (Being addressed by the live-update poll fix: yield during foreground load + target committed CURRENT_SOURCE.)
