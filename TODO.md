...
- [ ] (QA) `/api/manifest/signature` returns 400 on the live-update poll for git URLs (seen with nanostores@main, repeating 4×). The 400 is `resolve_source(src, branch)` raising `ResolveError(400)` in the signature route — likely the poll firing before the clone/resolve completes (timing race) or a resolve failure for that repo. Unrelated to building settings. Decide whether the poll should hold off until the repo is loaded.
- [ ] (QA) building glitch after a layout-dims rebuild — changing Min/Max width (a Rebuild-route setting → `applyManifest` re-runs → buildings rebuild) leaves black streaky artifacts on some buildings that flicker with camera angle (looks like stale OLD building positions z-fighting the new ones). Investigate the buildings reactive rebuild path: prior cell-root disposal, the enter/stay tween starting from stale `getMatrixAt` transforms (the diff reads prev transforms from the just-disposed mesh — `_computeBuildingDiff`), leftover instance matrices beyond the new per-cell count, or the shared material's per-instance attrs after a capacity change. Reported during QA with screenshots.
