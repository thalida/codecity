...
- [ ] pyright-strict debt in the data layer — `api/services/scan.py` (~22) + `api/services/cache.py` (~32) have pre-existing strict errors in untyped dict-walking (`compute_tree_signature`/`_walk`, `_coerce_file_entry`). Predates the FastAPI migration (the services moved as-is; strict was never green). Tighten the internal dict types so `uv run pyright` is clean.
- [ ] manifest wire-model optionality accuracy — the Pydantic `GitMeta.created/modified` and `RepoInfo.*` fields use `Optional[...] = None` (→ optional in OpenAPI), but the scanner always emits them (required-nullable, as the hand-written TS declares). Make them required-nullable so the OpenAPI/Scalar docs + generated types match the true wire; likewise reconsider `media_width/height` (absent-or-int, never null). `app/src/types/manifest.contract.ts` compares key sets so this passes today, but a deeper-equality guard would flag it.
