...
/** One city's building material: the ShaderMaterial every cell's detail mesh
 *  shares, plus the icon atlas bound to its uniforms.
 *
 *  A factory, not a module singleton, because a ShaderMaterial belongs to the
 *  WebGL context that compiled it and an icon atlas to the build that produced
 *  it. Two cities on one page sharing either is GPU-resource corruption, not a
 *  policy problem. */
export interface BuildingMaterial {
  /** Compiles on first call, then returns the same instance. */
  get(): THREE.ShaderMaterial;
  /** Pushed in before the cell pass, so buildings have it as they are built. */
  setIconAtlas(atlas: IconAtlas | null): void;
  /** The atlas cells resolve per-instance roof UVs from, or null before it
   *  builds. Single source, shared with the uIconAtlas uniform. */
  getIconAtlas(): IconAtlas | null;
  setTranslucent(on: boolean): void;
  /** Re-apply the committed config to the uniforms, so a Save reaches the whole
   *  city without rebuilding any of it. */
  refresh(): void;
  dispose(): void;
}

export function createBuildingMaterial(): BuildingMaterial {
  ...
  function setIconAtlas(atlas: IconAtlas | null): void {
    _atlas = atlas;
    if (_sharedMaterial) {
  ...
  function getIconAtlas(): IconAtlas | null {
    return _atlas;
  }
  ...
  function setBuildingsTranslucent(on: boolean): void {
  ...
  function getBuildingMaterial(): THREE.ShaderMaterial {
    ...
    _sharedMaterial = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uDimGlowColor: {
          value: setColorFromHex(new THREE.Color(), BUILDINGS.value.DIM_GLOW_COLOR),
        },
  ...
  function refreshBuildingMaterial(): void {
    ...
    _sharedMaterial.uniforms.uFogIntensity.value = sceneCfg.FOG_ENABLED
      ? sceneCfg.FOG_INTENSITY
      : 0;
  ...
  return {
    get: getBuildingMaterial,
    setIconAtlas,
    getIconAtlas,
    setTranslucent: setBuildingsTranslucent,
    refresh: refreshBuildingMaterial,
    dispose(): void {
