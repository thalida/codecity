// features/home/components/sources/HostingIcon/HostingIcon.tsx — Where a repo lives, as one glyph: the hosting
// provider's mark for a remote, a folder for a path on disk. The brand glyphs
// are inlined because lucide dropped brand icons; the two fallbacks are its
...
export function HostingIcon({ src }: HostingIconProps) {
  if (srcKind(src) === SourceKind.Local) return <Folder class="icon" aria-hidden="true" />;
  const lower = src.toLowerCase();
  if (/github\.com/.test(lower)) return <GithubGlyph />;
  // Any gitlab.* host, not gitlab.com alone: the projects worth showing off run
  // their own (gitlab.gnome.org, gitlab.freedesktop.org).
  if (/\bgitlab\.[a-z]/.test(lower) || /\.gitlab\.io/.test(lower)) return <GitlabGlyph />;
  if (/bitbucket\.org/.test(lower)) return <BitbucketGlyph />;
  // Everything else, self-hosted Forgejo and cgit included: a globe is honest
  // where a wrong brand mark would not be.
  return <Globe class="icon" aria-hidden="true" />;
}
