// features/city/components/FilePreviewPane/FilePreviewPane.tsx — body content for the right sidebar: an image,
...
function FileTextPreview({ file, source }: FileTextPreviewProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    API.fetchFileText(source, file.path, file.modified, scrub?.blobShaFor(file.path)).then(
      (text) => {
        if (cancelled) return;
  ...
  }, [source.src, file.path, file.modified, scrub?.blobShaFor(file.path)]);
...
function FontPreview({ file, source }: FontPreviewProps) {
  ...
  useEffect(() => {
    ...
    API.fetchFileBytes(source, file.path, file.modified, scrub?.blobShaFor(file.path)).then(
      async (buf) => {
        if (cancelled) return;
  ...
  }, [source.src, file.path, file.modified, family, scrub?.blobShaFor(file.path)]);
...
function _previewBody(file: FileNode | null, source: SourceRef | null, scrub: Scrub | null) {
  if (!file) {
    return (
      <PaneEmpty
  ...
  const url = API.fileUrl(source, file.path, file.modified, scrub?.blobShaFor(file.path));
...
export function FilePreviewPane({ state, onClose, onFocus, onExclude }: FilePreviewPaneProps) {
  ...
  const absent = Boolean(file && (isAbsent || scrub?.noContentAt(path)));
  ...
  return (
    <Pane
      ...
      footerSlot={
        file && !absent ? (
          <PaneStats items={fileStatItems(file, { scrubbed: scrub?.statsFor(file.path) })} />
        ) : null
      }
    >
      {absent ? (
        ...
        _previewBody(file, source, scrub)
      )}
    </Pane>
  );
}
