// features/city/state/timeline.ts — entering Timeline, with this app's overlay
// over it.
//
// The LOAD is the city's: the fetch, the pack, the bundle, the mode and
...
import { failHostWork, beginHostWork, setRebuildDetail } from '@/features/city/state/readout';
import {
  showLoadingOverlay,
  setLoadingStep,
  setLoadingStepTail,
...
import { setTimelineRefreshHandler } from '@/features/city/state/commands';
...
export async function loadTimelineSource(
  ...
  try {
    ...
    // The city installed the bundle as part of the load; this is the app's own
    // half — the project it now considers open.
