...
describe('createCity', () => {
  ...
  // Leaving timeline mode is what tears the union city down: a source switch
  // exits without touching the scene, so nothing else would.
  describe('Timeline-mode scene teardown', () => {
    ...
    it('uninstalls the scrub controller on leaving timeline mode', async () => {
      const handle = await createCity(makeCanvas());
