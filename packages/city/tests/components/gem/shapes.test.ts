...
describe('GEM.SIDES field derivation', () => {
  it('options are exactly the GEM_SHAPES keys, in order', () => {
    expect(GEM_FIELDS.SIDES.options.map((o) => o.value)).toEqual(Object.keys(GEM_SHAPES));
  });

  it('default is the canonical default shape', () => {
    expect(GEM_FIELDS.SIDES.default).toBe(GEM_SIDES_DEFAULT);
  });
});
