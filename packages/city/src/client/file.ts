...
export function createFileEndpoints(apiUrl: ApiUrl) {
  ...
  function fileUrl(source: SourceRef, path: string, mtime?: string, sha?: string | null): string {
  ...
  async function isContentPending(url: string): Promise<boolean> {
    try {
