...
export function createDiscoverEndpoints(apiUrl: ApiUrl) {
  function usable(entry: DiscoverEntry): boolean {
    return Boolean(entry?.url) && Boolean(entry?.label);
  }

  async function fetchDiscover(): Promise<readonly DiscoverEntry[]> {
    try {
   ...
   * roundtrip.
   */
  function getDiscover(): Promise<readonly DiscoverEntry[]> {
  ...
  function _resetDiscoverForTests(): void {
    _cached = null;
  }
