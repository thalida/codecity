...
export function createLayoutClient(): LayoutClient {
  ...
  function _computeSync(
    ...
    try {
      const layout = layoutCity(manifest as unknown as Parameters<typeof layoutCity>[0], cfg);
      queueMicrotask(() => {
  ...
  function compute(
    ...
    if (reuseLayoutFrom) {
      ...
      return new Promise<CityLayout>((resolve, reject) => {
        ...
        queueMicrotask(() => {
          ...
          pending.delete(id);
          try {
            resolve(reuseLayout(reuseLayoutFrom, manifest, cfg));
          } catch (err) {
            reject(err instanceof Error ? err : new Error(String(err)));
          }
        });
    ...
    return new Promise<CityLayout>((resolve, reject) => {
      ...
      if (!w) {
        _computeSync(id, manifest, cfg, resolve, reject);
        return;
      }
