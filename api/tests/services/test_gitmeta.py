...
from api.services.filemeta import epoch_to_iso
...
def test_build_authors_list(trailers, expected):
    from api.services.gitmeta import build_authors_list
...
class GitHistoryParallelTests(CacheRedirectMixin, unittest.TestCase):
    ...
    def test_single_walk_invocation(self):
        ...
        with (
            patch("api.services.gitmeta.subprocess.run", side_effect=counting_run),
            patch("api.services.gitmeta.subprocess.Popen", side_effect=counting_popen),
        ):
...
class GitLogRobustnessTests(CacheRedirectMixin, unittest.TestCase):
    ...
    def test_parse_loop_exception_kills_subprocess_before_waiting(self):
        ...
        from unittest.mock import patch
        from api.services.gitmeta import _walk_git_log
        ...
        fake = _FakeProc()
        with patch("api.services.gitmeta.subprocess.Popen", return_value=fake):
...
class GitHistoryCacheTests(CacheRedirectMixin, unittest.TestCase):
    ...
    def test_warm_run_skips_git_log(self):
        ...
        with patch("api.services.gitmeta.subprocess.run", side_effect=counting_run):
            collect_git_history(FIXTURE, use_cache=True)
    ...
    def test_cache_invalidated_after_new_commit(self):
        ...
        try:
            ...
            with (
                patch("api.services.gitmeta.subprocess.run", side_effect=counting_run),
                patch(
                    "api.services.gitmeta.subprocess.Popen", side_effect=counting_popen
                ),
            ):
                collect_git_history(FIXTURE, use_cache=True)
...
def test_history_as_of_ref_excludes_future_commits(tmp_path):
    from api.services.gitmeta import collect_git_history, run_git
