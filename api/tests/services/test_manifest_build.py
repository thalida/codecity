...
from api.services.manifest_build import sample_commits
...
def test_sample_commits_leaves_short_histories_alone(total, monkeypatch):
    monkeypatch.setattr("api.services.manifest_build.MAX_WIRE_COMMITS", 10)
...
def test_sample_commits_strides_a_deep_history(monkeypatch):
    monkeypatch.setattr("api.services.manifest_build.MAX_WIRE_COMMITS", 5)
...
def test_sample_commits_keeps_both_ends_at_the_cap_boundary(monkeypatch):
    monkeypatch.setattr("api.services.manifest_build.MAX_WIRE_COMMITS", 3)
