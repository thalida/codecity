...
describe('fileUrl', () => {
  it('names the repo and the path inside it, never an absolute one', () => {
    const url = new URL(fileUrl(TEST_SOURCE, 'src/a.ts'));
  ...
  it('carries the branch when the manifest was built with one', () => {
    const url = new URL(fileUrl({ src: 'https://host/o/r', branch: 'main' }, 'a.ts'));
    ...
    expect(new URL(fileUrl(TEST_SOURCE, 'a.ts')).searchParams.has('branch')).toBe(false);
  });

  it('omits the version param when none is given (unchanged URL)', () => {
    expect(fileUrl(TEST_SOURCE, 'a.ts')).not.toContain('v=');
  });

  it('adds a cache-busting mtime param carrying the given value', () => {
    const url = fileUrl(TEST_SOURCE, 'a.ts', '2026-07-19T12:00:00Z');
  ...
  it('a blob sha replaces the mtime buster: content-addressed needs no cache key', () => {
    const url = fileUrl(TEST_SOURCE, 'a.ts', '2026-07-19T12:00:00Z', 'a'.repeat(40));
