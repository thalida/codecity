...
describe('ageT()', () => {
  ...
  it('returns 0.5 when the range has zero span', () => {
    ...
    expect(ageT({ date: '2026-01-01', files: 1, sha: 'a'.repeat(40) }, zero)).toBe(0.5);
  });

  it('clamps out-of-range dates to [0,1]', () => {
    const range = computeAgeRange(commits);
    expect(ageT({ date: '2025-01-01', files: 1, sha: 'a'.repeat(40) }, range)).toBe(0);
    expect(ageT({ date: '2027-01-01', files: 1, sha: 'a'.repeat(40) }, range)).toBe(1);
  });
});
...
describe('sizeT()', () => {
  ...
  it('returns 0.5 when the range has zero span', () => {
    ...
    expect(sizeT({ date: '2026-01-01', files: 1, sha: 'a'.repeat(40) }, zero)).toBe(0.5);
  });
});
