...
function applyStructure(sceneState: ReturnType<typeof makeCityState>, layout: CityLayout): void {
  sceneState.layout.value = layout;
...
describe('scenic reactivity — structureRevision gates rebuilds', () => {
  ...
  it('streets: rebuilds on a structure change, skips on a reuse apply', () => {
    ...
    applyStructure(sceneState, makeLayout());
    ...
    sceneState.layout.value = makeLayout();
  ...
  it('gem: rebuilds on a structure change, skips on a reuse apply', () => {
    const sceneState = makeCityState();
    const gem = createGem(makeSceneContext(sceneState));
  ...
  it('island: resizes on a structure change, skips on a reuse apply', () => {
    const sceneState = makeCityState();
    const island = createIsland(makeSceneContext(sceneState));
  ...
  it('repoLabel: repositions on every manifest change (name + anchor)', () => {
    const sceneState = makeCityState();
    const label = createRepoLabel(makeSceneContext(sceneState), { getGem: () => null });
    disposers.push(() => label.dispose());

    sceneState.manifest.value = makeManifest('repo-a');
    ...
    sceneState.manifest.value = makeManifest('a-much-longer-repo-name-than-repo-a');
