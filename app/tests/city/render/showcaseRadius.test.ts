...
import { showcaseRadius } from '@/city/render/showcaseRadius';
...
describe('showcaseRadius', () => {
  it('sits at the default camera distance at 1', () => {
    expect(showcaseRadius(1, LIMITS, geometry(800))).toBe(800);
  });

  it('scales linearly, so half the value is half the distance', () => {
    expect(showcaseRadius(0.5, LIMITS, geometry(800))).toBe(400);
    expect(showcaseRadius(2, LIMITS, geometry(800))).toBe(1600);
  });
  ...
  it('holds the same framing across projects of wildly different size', () => {
    ...
    const small = showcaseRadius(0.1, LIMITS, geometry(600));
    const huge = showcaseRadius(0.1, LIMITS, geometry(900));
  ...
  it('never puts the camera inside the gem', () => {
    expect(showcaseRadius(0, LIMITS, geometry(800, 12))).toBe(12);
  });

  it('holds inside the limits a hand-driven camera has', () => {
    expect(showcaseRadius(2, { minDistance: 1, maxDistance: 50 }, geometry(800))).toBe(50);
    expect(showcaseRadius(0, { minDistance: 30, maxDistance: 50 }, geometry(800))).toBe(30);
  });

  it('falls back to the island before a city is built', () => {
    const radius = showcaseRadius(1, LIMITS, {
      gemRadius: null,
      gemFitDistance: null,
