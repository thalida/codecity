...
describe('rainbowRgbAt', () => {
  it('hue 0 at full saturation is pure red', () => {
    ...
    const [r, g, b] = rainbowRgbAt(1000, 0, RAINBOW.value);
  ...
  it('wraps a negative (t + fraction) into [0,1)', () => {
    RAINBOW.value = { SPEED: -1, SATURATION: 1, LIGHTNESS: 0.5 };
    // t = -0.25, which wraps to hue 0.75: violet, blue-dominant.
    const [r, g, b] = rainbowRgbAt(0.25, 0, RAINBOW.value);
  ...
  it('respects SPEED — time advances the hue', () => {
    RAINBOW.value = { SPEED: 0.001, SATURATION: 1, LIGHTNESS: 0.5 };
    const a = [...rainbowRgbAt(0, 0, RAINBOW.value)];
    const b = [...rainbowRgbAt(100, 0, RAINBOW.value)]; // t = 0.1 → hue 0.1, different color
  ...
  it('respects SATURATION and LIGHTNESS', () => {
    ...
    const [r, g, b] = rainbowRgbAt(123, 0.37, RAINBOW.value);
  ...
  it('the fraction offsets the hue (chase around the shape)', () => {
    ...
    const red = rainbowRgbAt(999, 0, RAINBOW.value); // hue 0 → red
    expect(red[0]).toBeCloseTo(1, 6);
    expect(red[1]).toBeCloseTo(0, 6);
    const third = [...rainbowRgbAt(999, 1 / 3, RAINBOW.value)]; // hue 1/3 → green
  ...
  it('writes into the provided out tuple and returns it', () => {
    ...
    const ret = rainbowRgbAt(0, 0, RAINBOW.value, out);
    expect(ret).toBe(out);
