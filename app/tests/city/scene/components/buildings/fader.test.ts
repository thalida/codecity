...
import { createBuildingFader } from '@/city/scene/components/buildings/fader';
...
describe('buildingFader → material transparency', () => {
  ...
  it('turns blending on only while the cascade is dimming something', () => {
    ...
    expect(MATERIAL.get().transparent).toBe(false);
    ...
    expect(MATERIAL.get().transparent).toBe(true);
