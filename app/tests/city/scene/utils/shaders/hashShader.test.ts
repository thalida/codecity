...
const SCENE_DIR = resolve(dirname(fileURLToPath(import.meta.url)), '../../../../../src/city/scene');
...
describe('hash.glsl (mobile-safe randomness)', () => {
  it('no city shader uses the sin-based hash', () => {
    const offenders = glslFiles(SCENE_DIR).filter((p) => {
  ...
  it('the chunk defines the four hashes shaders include', () => {
    const src = readFileSync(join(SCENE_DIR, 'utils/shaders/hash.glsl'), 'utf-8');
    for (const sig of [
