...
import { getWorldBounds } from '@/city/scene/utils/floorBounds';
...
describe('worldBounds', () => {
  ...
  it('returns fallback rectangle when bbox is null', () => {
    const b = getWorldBounds(null, WORLD.value);
    expect(b.cx).toBe(0);
    expect(b.cz).toBe(0);
  ...
  it('returns fallback when bbox is undefined', () => {
    const b = getWorldBounds(undefined, WORLD.value);
    expect(b.cx).toBe(0);
    expect(b.cz).toBe(0);
  });

  it('centers on bbox center', () => {
    const b = getWorldBounds(bbox(100, 200, 1100, 1200), WORLD.value);
  ...
  it('half-width is half the bbox width plus buffer', () => {
    ...
    const b = getWorldBounds(bbox(0, 0, 10000, 10000), WORLD.value);
  ...
  it('buffer applies the SAME absolute amount to both axes', () => {
    ...
    const b = getWorldBounds(bbox(0, 0, 10000, 100), WORLD.value);
    expect(b.halfWidth).toBe(8000);
  ...
  it('tiny cities get tiny bounds (no floor; slider always meaningful)', () => {
    ...
    const b = getWorldBounds(bbox(0, 0, 10, 10), WORLD.value);
  ...
  it('cityHeight feeds the characteristic-size calc for tiny-tall cities', () => {
    ...
    const b = getWorldBounds(bbox(0, 0, 100, 100), WORLD.value, 5000);
