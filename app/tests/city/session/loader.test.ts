...
describe('CityLoader loadSource cancellation', () => {
  ...
  describe('loading header label', () => {
    it('is up before the server sends one, from whoever opened it', async () => {
      session.progress.pendingLabel.value = null;

      // A recents row knows the label already; the URL adapter does not, and
      // then the overlay names the source until the first event lands.
      const p = session.load.loadSource({ src: 'https://github.com/o/r', label: 'o/r' });
