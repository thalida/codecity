...
describe('committing a source', () => {
  ...
  beforeEach(() => {
    ...
    // what pointing it at this one produces.
    stopUrl = attachUrlBinding(session);
  });
  ...
  describe('the history entry it leaves', () => {
    ...
    it('PUSHES when the project was opened from the switcher, so Back returns to it', () => {
      ...
      session.source.commit('https://github.com/o/r', undefined, MANIFEST_FOR);
    ...
    it('replaces when already on a city, since a re-scan is the same place', () => {
      ...
      session.source.current.value = null;
      const push = vi.spyOn(history, 'pushState');
      session.source.commit('https://github.com/o/other', undefined, MANIFEST_FOR);
  ...
  it('resolves the branch once and gives the load ONE identity', () => {
    ...
    session.source.commit('https://github.com/o/r', undefined, {
      tree: { name: 'r' },
      repo: { branch: 'main' },
  ...
  it('keeps that one identity when Timeline recommits the same source', () => {
    ...
    session.source.commit('https://github.com/o/r', session.source.current.value?.branch, {
      tree: { name: 'r' },
  ...
  it('records an explicitly-requested branch', () => {
    session.source.commit('https://github.com/o/r', 'dev', {
      tree: { name: 'r' },
      repo: { branch: 'dev' },
    } as unknown as Manifest);
  ...
  it('records a local source with no branch (branch is not part of its identity)', () => {
    ...
    session.source.commit('/Users/me/worktrees/feat-x', undefined, {
  ...
  it('dedupes a local path across checkouts into one recent', () => {
    ...
    session.source.commit('/proj', undefined, {
      tree: { name: 'proj' },
      repo: { branch: 'main' },
    } as unknown as Manifest);
    session.source.commit('/proj', undefined, {
  ...
  it('drops the branch from CURRENT_SOURCE + the URL for a local source', () => {
    ...
    session.source.commit('/Users/me/proj', 'stale-branch', {
      tree: { name: 'proj' },
      repo: { branch: 'main' },
    } as unknown as Manifest);
