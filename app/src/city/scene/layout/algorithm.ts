...
export function estimateDirReaches(
  ...
  const streetLayout = cfg.streetLayout;
  ...
  const bldgDims = cfg.buildingDimensions;
  const distFromRoad = bldgDims.DISTANCE_FROM_ROAD;
  const gemSizing = cfg.gemSizing;
  ...
  const myStreetWidth = _streetWidthForDir(dir, cfg.streetTiers);
...
function _layoutDir(
  ...
  for (const child of children) {
    if (child.type === NodeKind.File) {
      ...
      const subStreetWidth = _streetWidthForDir(child as DirLike, cfg.streetTiers);
...
function _layoutCityInternal(
  ...
  estimateDirReaches(tree, stats.lines, stats.bytes, undefined, reachCache, cfg);
