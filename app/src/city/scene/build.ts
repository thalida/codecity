// city/scene/build.ts — what the last apply MADE of this city, and the pipeline
// that makes it: manifest and layout are source signals reassigned every apply,
// the revisions are change counters, the rest are computed. Components rebuild
// off them. Per scene, so a second city builds without disturbing this one.
...
import { type BuildReporter } from '@/city/session/stores/progress';
import { BuildStage } from '@/constants/progress';
import { StreetAxis } from '@/city/scene/types';
import type { Manifest } from '@/types';
import type { Building, CityBbox, CityLayout, Street } from '@/city/scene/types';
...
import { gemAnchorXZ } from '@/city/scene/components/gem/anchor';
...
export function createCityBuild(
  ...
  // Change-notification counters (see CityBuild for what each means + who tracks it).
