...
import type { StreetsConfig } from '@/city/session/settings/streets';
import type { RuinsConfig } from '@/city/session/settings/ruins';
import { ASPHALT_WIDTH_FRAC } from '@/city/scene/constants/streets';
...
import { CapStyle, JoinSide, StreetAxis } from '@/city/scene/types';
...
export function createMergedSidewalkMesh(
  ...
  yBase: number,
  streetsCfg: StreetsConfig,
  ruinsCfg: RuinsConfig
  ...
  const def = new THREE.Color(streetsCfg.SIDEWALK_DEFAULT);
  ...
  setColorFromHex(ruinUniform.value, ruinsCfg.SIDEWALK_COLOR);
...
export function createMergedAsphaltMesh(
  ...
  merged.setAttribute('aRuin', new THREE.BufferAttribute(new Float32Array(vAcc), 1));
  const mat = flatGroundMaterial(streetsCfg.ASPHALT_COLOR, RENDER_ORDERS.ASPHALT);
  ...
  const ruinUniform = { value: new THREE.Color() };
  setColorFromHex(ruinUniform.value, ruinsCfg.ROAD_COLOR);
  injectStreetOpacity(mat, { ruin: ruinUniform });
  mat.userData.uRuinColor = ruinUniform;
  const mesh = new THREE.Mesh(merged, mat) as FlatMesh;
