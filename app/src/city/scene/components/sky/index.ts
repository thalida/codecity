...
import { CAMERA_FAR } from '@/city/scene/constants/camera';
...
export function createSky(ctx: SceneContext): Sky {
  ...
  const cfg = ctx.config.SCENE.value;
  ...
  // Pushes this city's scene settings into the uniforms with no rebuild. Runs
  // once at construction too, re-applying what the constructor already baked.
  const stopEffect = onConfig(ctx.config.SCENE, () => {
    const c = ctx.config.SCENE.value;
