...
import type { StreetsConfig, StreetTiersConfig } from '@/city/session/settings/streets';
...
import { PATH_LINE_ELEVATION, HOVER_PATH_LINE_ELEVATION } from '@/city/scene/constants/streets';
...
export function createPathLineRenderer({
  ...
  function _updatePathLine(): void {
    const sel = picker.selection.value;
    const gemPos = build.gemWorldPos.peek();
    if (!gemPos || !sel) {
      pathLine.visible = false;
    ...
    const pts = computePathPoints(sel, { x: gemPos.x, z: gemPos.z }, build.streetsByDirMap.peek());
    if (pts.length < 2) {
      pathLine.visible = false;
      pathLineMat.opacity = 0;
      pathSegmentCount = 0;
  ...
  function _updateHoverPathLine(): void {
    ...
    const pts = computePathPoints(hov, { x: gemPos.x, z: gemPos.z }, build.streetsByDirMap.peek());
