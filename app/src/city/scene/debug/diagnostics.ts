...
// through. The entry points read build; the formatters read nothing but
// their arguments, so they test in isolation.

import * as THREE from 'three';
import { layoutCityWithTrace } from '@/city/scene/layout/algorithm';
import type { LayoutConfig } from '@/city/scene/layout/protocol';
import { findLayoutOverlaps, LayoutOverlapCategory } from '@/city/scene/layout/overlaps';
import type { LayoutOverlap } from '@/city/scene/layout/overlaps';
import type { ChildPlacementTrace, StemPlacementTrace } from '@/city/scene/layout/stemSolver';
import type { WorldRect } from '@/city/scene/layout/occupancyIndex';
import type { CityBuild } from '@/city/scene/build';

// Unexpected overlaps warn with a line each; a clean run logs a summary.
export function runCollisionCheck(build: CityBuild): void {
  const layout = build.layout.value;
  if (!layout) {
...
export function runStemPlacementDiagnostic(build: CityBuild, cfg: LayoutConfig): void {
  const m = build.manifest.value;
  if (!m) {
