...
import { until } from '@/city/scene/utils/until';
...
import { createScrubController } from './scrub/scrubController';
...
import { createCityBuild } from './build';
import {
  runCollisionCheck,
...
import { showTooltip, hideTooltip } from './interaction/tooltip/tooltip';
...
export async function createCityScene(
  ...
  // Pulled off build for the City handle; components never wire into
  // these — they rebuild reactively off build's signals.
  const { applyManifest, buildStagesFor, invalidateLayoutCache } = build;
  ...
  // The rig reads its framing inputs from build directly; deps carries
  ...
  const rig = createCameraRig({
    ...
    deps: {
      ...
      getRepoLabelBounds: () =>
        repoLabelBounds(
          build.manifest.peek()?.tree?.name,
          build.gemWorldPos.peek(),
  ...
  const stopReframe = effect(() => {
    ...
    if (build.manifest.peek() === null) return;
  ...
  const stopOnScreen = effect(() => {
    const revision = build.cityRevision.value;
    if (build.manifest.peek() === null) return;
    untracked(() => {
  ...
  function whenOnScreen(): Promise<void> {
    return until(() => gone.value || presented.value >= build.cityRevision.value);
  }
  ...
  const picker = createPicker({
    ...
    world: {
      ...
      getStreetByDir: (p) => build.streetsByDirMap.peek()[p] ?? null,
  ...
  const timelineApi = {
    installScrubController(
      ...
      _scrubController = createScrubController({
        ...
        timelines,
        commitLineRanges,
        heightCtx: makeHeightContext(build.manifest.peek()?.stats),
        scannedAt: build.manifest.peek()?.scanned_at,
        streetsByDir: build.streetsByDirMap.peek(),
        scrubGates: [
          {
  ...
  async function repack(): Promise<void> {
    ...
    const showing = build.manifest.peek();
  ...
  return {
    ...
    world: {
      getTrees: () => trees.getRenderer(),
      runCollisionCheck: () => runCollisionCheck(build),
      runStemPlacementDiagnostic: () =>
        runStemPlacementDiagnostic(build, {
