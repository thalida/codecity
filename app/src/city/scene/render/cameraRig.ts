...
import { gemRadiusFor } from '@/city/scene/components/gem/mesh';
...
export function createCameraRig({
  ...
  deps: CameraRigDeps;
  build: CityBuild;
  /** What this city looks like: its opening pose and its gem's size. */
  config: CityConfig;
  mode?: CameraMode;
}) {
  ...
  const backdropPose = computed(() => {
    const s = config.HOME_BACKDROP.value;
  ...
  function _captureFraming(): boolean {
    ...
    const gemPos = build.gemWorldPos.value;
    const rootStreet = build.rootStreet.value;
    ...
    const dir = computeFramingDir(
      config.CAMERA.value.ELEVATION,
      config.CAMERA.value.AZIMUTH,
    ...
    let heightDist = 0;
    const tallest = gemPos && rootStreet ? build.tallestBuilding.value : null;
  ...
  function _focusTopDown(
    ...
    if (horizLenSq < 1e-4) {
      const root = build.rootStreet.value;
  ...
  function captureView(opts: {
    ...
    const dir = computeFramingDir(
      opts.elevation,
      opts.azimuth,
      build.rootStreet.value?.orientation ?? null
    );
    camera.up.set(0, 1, 0);
  ...
  function captureAnchors(): {
    ...
    const gem = build.gemWorldPos.value;
    const tb = build.tallestBuilding.value;
    const bbox = build.bbox.value;
    ...
    return {
      ...
      cityRadius,
      rootStreetWidth: build.rootStreet.value?.width ?? 0,
    };
  }
  ...
  function streetAnchor(
    ...
    const s = build.streetsByDirMap.value[path];
    if (!s) return null;
  ...
  function _backdropRadius(distance: number): number {
    ...
    return backdropRadius(distance, controls, {
      gemRadius: rootStreet ? gemRadiusFor(rootStreet.width, config.GEM_SIZING.value) : null,
      gemFitDistance: _gemFitDistance(),
      worldBounds: build.latestWorldBounds.value,
    });
  }
  ...
  function _backdropPose(): CameraPlacement | null {
    const gem = build.gemWorldPos.value;
    if (!gem) return null;
    const backdrop = config.HOME_BACKDROP.value;
    const dir = computeFramingDir(
      backdrop.ELEVATION,
      backdrop.AZIMUTH,
      build.rootStreet.value?.orientation ?? null
  ...
  const _disposeBackdropSpeedEffect = effect(() => {
    controls.autoRotateSpeed = config.HOME_BACKDROP.value.ROTATE_SPEED;
  });
