...
import { nextPaint } from '@/city/scene/utils/nextPaint';
...
export class TimelineMode {
  ...
  loadSource = async ({
    ...
    if (overlay) {
      ...
      this.session.progress.pendingLabel.value = this.session.source.info.peek().label || null;
    ...
    try {
      ...
      this.session.source.set(src, branch, manifest);
