...
const RAINBOW_FIELDS = {
  SPEED: { route: ChangeRoute.Live, kind: FieldKind.Slider, default: 0.0005, min: 0, max: 0.005, step: 0.0001, label: 'Speed',
  ...
  SATURATION: { route: ChangeRoute.Live, kind: FieldKind.Slider, default: 1.0, min: 0, max: 1, step: 0.05, label: 'Saturation' },
  LIGHTNESS: { route: ChangeRoute.Live, kind: FieldKind.Slider, default: 0.5, min: 0, max: 1, step: 0.05, label: 'Lightness' },
} satisfies FieldMap;
...
const BLOOM_FIELDS = {
  ...
  WINDOW_EMISSION: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 1.0, min: 0, max: 3.0, step: 0.05, label: 'Window emission',
  ...
  GEM_EMISSION: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.5, min: 0, max: 5.0, step: 0.1, label: 'Gem emission',
  ...
  AD_EMISSION: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.6, min: 0, max: 5.0, step: 0.1, label: 'Ad emission',
  ...
  STRENGTH: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.1, min: 0, max: 1, step: 0.01, label: 'Strength',
    tip: 'Overall bloom intensity multiplier. 0 = bloom pass produces nothing; 1 = full strength.' },
  RADIUS: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 1.0, min: 0, max: 1.0, step: 0.05, label: 'Radius',
    tip: "How far each bright pixel's glow spreads. Lower = tighter halos; higher = soft diffuse glow." },
  THRESHOLD: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.5, min: 0, max: 2.0, step: 0.05, label: 'Threshold (cutoff)',
