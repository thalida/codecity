// state/stores/build.ts — how the world build is going: which stage, whether it
// landed, and what it was built FROM. A different axis from the manifest itself,
// which is why a manifest can be current while the city on screen is not yet.
// Session-scoped: a rehydrated status would strand it on "rebuilding…".
