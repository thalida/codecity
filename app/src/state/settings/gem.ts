...
const GEM_FIELDS = {
  // ── Shape ──
  SIDES: { kind: FieldKind.Select, default: '8', label: 'Sides',
    options: [
      { value: '4', label: '4' },
      { value: '8', label: '8' },
      { value: '20', label: '20' },
  ...
  // ── Appearance ──
  EDGE_COLOR: { kind: FieldKind.Color, default: '#ffffff', label: 'Edge color',
    tip: 'Neutral separator line drawn around each gem face.' },
  BODY_OPACITY: { kind: FieldKind.Slider, default: 0.75, min: 0, max: 1, step: 0.05, label: 'Body opacity',
    tip: 'Gem transparency. Low = jewel-like; high = plastic.' },

  // ── Face colors ──
  FACE_1: { kind: FieldKind.Color, default: faceHex(0), label: 'Face 1' },
  FACE_2: { kind: FieldKind.Color, default: faceHex(1), label: 'Face 2' },
  FACE_3: { kind: FieldKind.Color, default: faceHex(2), label: 'Face 3' },
  FACE_4: { kind: FieldKind.Color, default: faceHex(3), label: 'Face 4' },
  FACE_5: { kind: FieldKind.Color, default: faceHex(4), label: 'Face 5' },
  FACE_6: { kind: FieldKind.Color, default: faceHex(5), label: 'Face 6' },
  FACE_7: { kind: FieldKind.Color, default: faceHex(6), label: 'Face 7' },
  FACE_8: { kind: FieldKind.Color, default: faceHex(7), label: 'Face 8' },

  // ── Glow halo (sub-feature) ──
  GLOW_ENABLED: { kind: FieldKind.Toggle, default: true, label: 'Enabled',
    tip: 'Two billboarded sprites behind the gem painted with a soft radial-gradient — creates a fuzzy neon halo.' },
  GLOW_INNER_SCALE: { kind: FieldKind.Slider, default: 4.0, min: 1, max: 12, step: 0.1, label: 'Inner scale × radius',
  ...
  GLOW_INNER_OPACITY: { kind: FieldKind.Slider, default: 0.75, min: 0, max: 1, step: 0.05, label: 'Inner opacity',
    tip: 'Brightness of the hot core. Lower for a subtler halo.' },
  GLOW_OUTER_SCALE: { kind: FieldKind.Slider, default: 15.0, min: 1, max: 30, step: 0.5, label: 'Outer scale × radius',
  ...
  GLOW_OUTER_OPACITY: { kind: FieldKind.Slider, default: 0.5, min: 0, max: 1, step: 0.05, label: 'Outer opacity',
    tip: 'Brightness of the atmospheric halo.' },
  GLOW_ANIMATE_COLORS: { kind: FieldKind.Toggle, default: true, label: 'Animate colors',
    tip: 'Cycle the halo color through the gem face palette. Off = halo uses the edge color from Appearance above.' },
  GLOW_CYCLE_PERIOD_SECONDS: { kind: FieldKind.Slider, default: 10.0, min: 1, max: 30, step: 0.5, label: 'Cycle period (s)',
    tip: 'Seconds for one full pass through every palette color. Below 1s reads as flicker; above 30s the cycle feels static.' },

  // ── Animation ──
  ROTATION_SPEED: { kind: FieldKind.Slider, default: 1.0, min: 0, max: 3, step: 0.05, label: 'Rotation speed',
    tip: 'Radians per second. Above 3 rad/sec the gem looks frantic.' },
  BOB_FREQUENCY: { kind: FieldKind.Slider, default: 1.0, min: 0, max: 5, step: 0.1, label: 'Bob frequency',
    tip: 'How fast the gem oscillates vertically. Above 5 cycles/sec it reads as vibration, not bobbing.' },
  BOB_AMPLITUDE_FRAC: { kind: FieldKind.Slider, default: 0.5, min: 0, max: 2, step: 0.05, label: 'Bob amplitude',
    tip: 'Vertical bob distance, as a fraction of the gem radius. Above 2× radius the gem flies off the street.' },
  HOVER_SCALE: { kind: FieldKind.Slider, default: 1.25, min: 1, max: 3, step: 0.05, label: 'Hover scale',
    tip: 'Multiplier applied to the gem when the cursor is over it. Above 3× the gem dominates the scene on hover.' },
  SCALE_LERP_SPEED: { kind: FieldKind.Slider, default: 0.1, min: 0.01, max: 1, step: 0.01, label: 'Hover lerp',
...
const GEM_SIZING_FIELDS = {
  RADIUS_AS_STREET_FRAC: { kind: FieldKind.Slider, default: 0.5, min: 0.05, max: 1, step: 0.05, label: 'Radius × street width',
    tip: 'Gem radius relative to the root street width. Bigger gems demand more empty plaza space.' },
  MIN_RADIUS: { kind: FieldKind.Number, default: 8, min: 1, max: 50, step: 1, label: 'Min radius',
...
const REPO_LABEL_FIELDS = {
  ...
  HEIGHT_PCT: { kind: FieldKind.Slider, default: 85, min: 0, max: 200, step: 1, label: 'Height % of max building',
  ...
  FONT_SIZE: { kind: FieldKind.Slider, default: 128, min: 10, max: 300, step: 1, label: 'Font size',
  ...
  ANIMATION_SPEED: { kind: FieldKind.Slider, default: 1.0, min: 0, max: 4, step: 0.05, label: 'Animation speed',
    tip: 'Multiplier on the holographic scanline / glitch rate. 0 freezes the label; 4 reads as frantic.' },
  OPACITY: { kind: FieldKind.Slider, default: 0.9, min: 0, max: 1, step: 0.05, label: 'Opacity',
    tip: 'Master opacity. 0 invisible, 1 fully painted.' },
  BEAM_COLOR: { kind: FieldKind.Color, default: '#bfb3ff', label: 'Beam color',
    tip: 'Color of the light beam rising from the gem.' },
  TEXT_COLOR: { kind: FieldKind.Color, default: '#ffffff', label: 'Text color',
