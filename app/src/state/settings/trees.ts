...
const TREES_FIELDS = {
  ENABLED: { kind: FieldKind.Toggle, default: true, label: 'Trees enabled',
  ...
  DENSITY_FALLOFF: { kind: FieldKind.Slider, default: 1.5, min: 0, max: 50, step: 0.1, label: 'Density falloff',
  ...
  COLOR_BUSY_DAY: { kind: FieldKind.Color, default: '#001908', label: 'Busy-day color',
    tip: 'Color for commits on a busy day — many commits sharing the same date. Live.' },
  COLOR_SOLO_DAY: { kind: FieldKind.Color, default: '#5a370a', label: 'Solo-day color',
    tip: 'Color for commits on a solo day — only one commit that date. Live.' },
  TRUNK_COLOR: { kind: FieldKind.Color, default: '#110c08', label: 'Trunk color',
    tip: 'Color of every tree trunk. Live.' },
  SHADING_STRENGTH: { kind: FieldKind.Slider, default: 0.65, min: 0, max: 1, step: 0.05, label: 'Shading strength',
    tip: 'Baked vertex-color gradient depth on the canopy. 0 = flat (no shading), 1 = fully dark at the base. Rebuild on change.' },

  AGE_DESAT_ENABLED: { kind: FieldKind.Toggle, default: false, label: 'Age desaturation enabled',
    tip: 'When on, older commits fade toward gray — newest commits keep full color, oldest are washed out. Live.' },
  AGE_SATURATION: { kind: FieldKind.RangePair, default: [50, 100] as [number, number], min: 0, max: 100, step: 1, label: 'Saturation range',
  ...
  MIN_HEIGHT: { kind: FieldKind.Slider, default: 8, min: 4, max: 400, step: 4, label: 'Min height',
  ...
  MAX_HEIGHT: { kind: FieldKind.Slider, default: 96, min: 16, max: 800, step: 4, label: 'Max height',
  ...
  MIN_WIDTH: { kind: FieldKind.Slider, default: 32, min: 2, max: 400, step: 2, label: 'Min canopy width',
  ...
  MAX_WIDTH: { kind: FieldKind.Slider, default: 64, min: 4, max: 600, step: 2, label: 'Max canopy width',
  ...
  TRUNK_RADIUS_FRAC: { kind: FieldKind.Slider, default: 0.15, min: 0.05, max: 0.5, step: 0.01, label: 'Trunk thickness (% of canopy)',
    tip: 'Trunk XZ radius as a fraction of canopy radius. Wider canopies get thicker trunks proportionally. Rebuild on change.' },
  WIDTH_AGE_FLOOR: { kind: FieldKind.Slider, default: 0.5, min: 0, max: 1, step: 0.05, label: 'Age shrink floor',
  ...
  FACETS_LOW: { kind: FieldKind.Slider, default: 5, min: 3, max: 24, step: 1, label: 'Low-tier facets',
  ...
  FACETS_MID: { kind: FieldKind.Slider, default: 8, min: 3, max: 24, step: 1, label: 'Mid-tier facets',
    tip: 'Radial segment count for the middle-commit tier.' },
  FACETS_HIGH: { kind: FieldKind.Slider, default: 12, min: 3, max: 32, step: 1, label: 'High-tier facets',
  ...
  OUTLINE_WIDTH: { kind: FieldKind.Number, default: 1, min: 1, max: 10, step: 1, label: 'Linewidth',
    tip: 'Pixel thickness shared by hover and selected canopy outlines.' },
  OUTLINE_HOVER_COLOR: { kind: FieldKind.Color, default: '#ffffff', label: 'Hover color',
    tip: 'Outline color when a tree is hovered (not selected).' },
  OUTLINE_HOVER_OPACITY: { kind: FieldKind.Slider, default: 0.5, min: 0, max: 1, step: 0.05, label: 'Hover opacity' },
  OUTLINE_SELECTED_OPACITY: { kind: FieldKind.Slider, default: 0.75, min: 0, max: 1, step: 0.05, label: 'Selected opacity',
