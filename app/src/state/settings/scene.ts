...
const SCENE_FIELDS = {
  // ── Sky ──
  SKY_COLOR: { route: ChangeRoute.Refresh, kind: FieldKind.Color, default: '#010005', label: 'Sky color',
  ...
  // ── Stars ──
  STARS_ENABLED: { route: ChangeRoute.Refresh, kind: FieldKind.Toggle, default: true, label: 'Enabled',
    tip: 'When off, no stars are sampled (also disables twinkle).' },
  STARS_DENSITY: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.0075, min: 0, max: 0.02, step: 0.0005, label: 'Density',
    tip: 'Hash-threshold for star presence — higher density paints MORE stars. Above ~0.01 the sky reads as a noise field.' },

  // ── Ground haze (fog) ──
  FOG_ENABLED: { route: ChangeRoute.Refresh, kind: FieldKind.Toggle, default: true, label: 'Enabled',
    tip: "Off → no haze (the shader's fog mix is a no-op). Other knobs stay in config so flipping back restores the mood." },
  FOG_COLOR: { route: ChangeRoute.Refresh, kind: FieldKind.Color, default: '#0f0821', label: 'Color',
    tip: 'Tint that building bases mix toward. Match the sky/ground for a seamless horizon.' },
  FOG_INTENSITY: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.8, min: 0, max: 1, step: 0.05, label: 'Intensity',
    tip: 'Peak fog amount at world Y=0 (street level). 0 = off; 1 = ground plane fully tinted to fog color.' },
  FOG_HEIGHT_FRAC: { route: ChangeRoute.Refresh, kind: FieldKind.Slider, default: 0.5, min: 0, max: 1, step: 0.05, label: 'Falloff height ×',
...
const WORLD_FIELDS = {
  GROUND_BUFFER_PERCENT: { route: ChangeRoute.Rebuild, kind: FieldKind.Slider, default: 0, min: 0, max: 100, step: 1, label: 'Ground buffer (% of city)',
