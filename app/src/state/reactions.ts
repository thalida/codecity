...
// LIGHTING is the only store not yet converted to schema-driven settingSignal;
// its refresh routing is hand-listed below until it's migrated (its own
// LIGHTING-eviction workstream). Every converted store routes automatically
// from its per-field `route` metadata via routeSignature() — no manual entry.
import { LIGHTING } from '@/state/settings/index';
...
// Legacy (not-yet-schema) LIGHTING routing, hand-listed until it's converted.
// Returns a JSON string that changes iff a LIGHTING key changes; the refresh
// computed below concatenates this with the auto-generated routeSignature().
// LIGHTING is material-refresh only — no rebuild contribution.
function legacyRefreshSig(): string {
  return JSON.stringify({ lighting: LIGHTING.value });
}
